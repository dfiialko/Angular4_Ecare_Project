...
export class DataService {
  ...
  getUsers() {
    ...
    const headers = new Headers({ 'Authorization': 'Bearer ' + localStorage.getItem('auth_token') });
  ...
  getTasks() {
    const headers = new Headers({ 'Authorization': 'Bearer ' + localStorage.getItem('auth_token') });
    const options = new RequestOptions({ headers: headers });
    return this._http.get('http://208.75.74.123:10020/api/tasks', options).
      map((response: Response) => response.json());
  }
  ...
  createTask() {
    const headers = new Headers({ 'Authorization': 'Bearer ' + localStorage.getItem('auth_token') });
    const options = new RequestOptions({ headers: headers });
    return this._http.post('http://208.75.74.123:10020/api/tasks', options).
      map((response: Response) => response.json());
  }
